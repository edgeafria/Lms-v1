...
const CoursesPage: React.FC = () => {
  ...
  >(null);
  const API_BASE_URL =
    import.meta.env.VITE_API_BASE_URL || "http://localhost:5000/api"; // Backend URL
    ...
    const fetchCourses = async () => {
      ...
      try {
        ...
        if (axios.isAxiosError(err)) {
          setError(
            `Network Error: ${err.message}. Please ensure the backend server at ${API_BASE_URL} is running.`
          );
        } else {
  ...
  const renderContent = () => {
    if (isLoading) {
      return (
        ...
        >
          ...
          <p className="ml-4 text-gray-600 text-lg font-body">
            Loading courses...
          </p>
        </div>
      );
    ...
    if (error) {
      return (
        <div
          className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded max-w-3xl mx-auto text-center shadow-md font-body"
          role="alert"
        >
        ...
        <div className="col-span-full text-center py-16 px-6 bg-white rounded-lg shadow font-body">
          ...
          >
            ...
            <path
              ...
              strokeLinejoin="round"
              strokeWidth="2"
              d="M15.5 15.5L19 19M9 13h6M9 9h6m-6-4h.01"
              opacity="0.4"
            />
          </svg>
          ...
          <p className="mt-1 text-sm text-gray-500">
            There are currently no published courses available. Please try again
            later!
          </p>
        </div>
      );
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="relative">
                <button className="flex items-center space-x-2 px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 font-body">
                  <Filter className="h-4 w-4" /> <span>All Categories</span>{" "}
        ...
        {!isLoading && !error && (
          <div className="mb-6">
            <p className="text-gray-600 font-body">
              ...
              {pagination
                ? `${
                    (pagination.currentPage - 1) * COURSES_PER_PAGE + 1
                  }-${Math.min(
                    pagination.currentPage * COURSES_PER_PAGE,
                    pagination.totalCourses
                  )} of ${pagination.totalCourses}`
                : courses.length}{" "}
              courses
            </p>
          </div>
