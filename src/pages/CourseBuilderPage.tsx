...
const CourseBuilder: React.FC = () => {
        ...
        .then(response => {
            if(response.data.success) {
                ...
                setCourseData({
                    ...
                    shortDescription: data.shortDescription || '', originalPrice: data.originalPrice, duration: data.duration,
  ...
  const handleSaveCourse = async (silent: boolean = false): Promise<CourseData | null> => {
     ...
     try {
        if (courseData.title.length < 5 || courseData.description.length < 50 || !courseData.category) {
            setActiveTab('basic');
            throw new Error("Validation Failed: Please fill in Title (min 5), Description (min 50), and Category.");
        }
         const dataToSave = {
            title: courseData.title, description: courseData.description, shortDescription: courseData.shortDescription, category: courseData.category, level: courseData.level, price: courseData.price, originalPrice: courseData.originalPrice, duration: courseData.duration,
         ...
         setCourseData(prev => {
            ...
            const newCourseData = { ...prev, id: savedData._id, status: savedData.status, thumbnail: prev.thumbnailFile ? prev.thumbnail : (savedData.thumbnail?.url || prev.thumbnail), requirements: (savedData.requirements || []).map((r: string, i: number) => ({ id: `req_${Date.now()}_${i}`, value: r })), learningOutcomes: (savedData.learningOutcomes || []).map((o: string, i: number) => ({ id: `out_${Date.now()}_${i}`, value: o })), tags: (savedData.tags || []).map((t: string, i: number) => ({ id: `tag_${Date.now()}_${i}`, value: t })), modules: finalModules };
  ...
  const handlePublish = async () => {
     const currentCourseId = courseData.id || courseId; if (!currentCourseId) { setError("You must save the course before publishing."); return; } setIsPublishing(true); setError(null); setMessage(null);
     try {
        const response = await axiosInstance.post(`/courses/${currentCourseId}/publish`);
        if (response.data.success) { const newStatus = response.data.data.status; setMessage(`Course successfully ${newStatus ? newStatus.charAt(0).toUpperCase() + newStatus.slice(1) : 'Updated'}!`); setCourseData(prev => ({ ...prev, status: newStatus })); } else throw new Error(response.data.message || 'Failed to update publish status');
     } catch (err) { console.error("Error updating publish status:", err); if (axios.isAxiosError(err)) setError(err.response?.data?.message || err.message); else if (err instanceof Error) setError(err.message); else setError('An unexpected error occurred.'); } finally { setIsPublishing(false); }
  };
  const addModule = () => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input type="text" value={courseData.title} onChange={(e) => setCourseData(prev => ({ ...prev, title: e.target.value }))} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="Enter course title" />
        </div>
        <div>
          <label className="block text-sm font-body font-medium text-gray-700 mb-2"> Category * </label>
          <select value={courseData.category} onChange={(e) => setCourseData(prev => ({ ...prev, category: e.target.value }))} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500">
        ...
        <div>
          ...
          <select value={courseData.level} onChange={(e) => setCourseData(prev => ({ ...prev, level: e.target.value as any }))} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500">
      ...
      <div>
        ...
        <textarea rows={6} value={courseData.description} onChange={(e) => setCourseData(prev => ({ ...prev, description: e.target.value }))} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="Describe what students will learn in this course" />
      </div>
       <div>
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           <div>
              ...
              <input type="number" value={courseData.originalPrice || ''} onChange={(e) => setCourseData(prev => ({ ...prev, originalPrice: Number(e.target.value) || undefined }))} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="e.g., 20000 (for showing discounts)" />
           </div>
           <div>
               <label className="block text-sm font-body font-medium text-gray-700 mb-2"> Course Duration (Optional) </label>
  ...
  const renderCurriculum = () => (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {courseData.modules.sort((a,b) => (a.order ?? 0) - (b.order ?? 0)).map((module, moduleIndex) => ( // Sort with fallback
          <div key={module.id} className="bg-white border border-gray-200 rounded-lg">
            <div className="p-4 border-b border-gray-200 bg-gray-50">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 sm:gap-4">
                <div className="flex items-center space-x-3 flex-1 min-w-0"> {/* Allow shrinking */}
                  ...
                  <div className="flex-1 min-w-0"> {/* Ensure inputs can shrink */}
                ...
                <div className="flex items-center space-x-2 flex-shrink-0">
                    <button onClick={() => addLesson(module.id)} className="text-primary-500 hover:text-primary-600 p-2 rounded-lg hover:bg-primary-50 flex items-center space-x-1 text-sm">
                        <Plus className="h-4 w-4" /> <span className="hidden sm:inline">Add Lesson</span> {/* Hide text on mobile */}
                    </button>
            ...
            <div className="p-2 sm:p-4"> {/* Less padding on mobile */}
              {module.lessons.length === 0 ? (
                ...
                <div className="space-y-3">
                  {module.lessons.sort((a,b) => (a.order ?? 0) - (b.order ?? 0)).map((lesson, lessonIndex) => { // Sort with fallback
                    const lessonKey = lesson.id || `fallback_${module.id}_${lessonIndex}`;
                    const lessonType = (lesson.type && ['video', 'text', 'quiz', 'assignment'].includes(lesson.type)) ? lesson.type : 'video'; // Fallback type for rendering
                    ...
                    <div key={lessonKey} className="flex flex-wrap items-start sm:items-center justify-between gap-3 sm:gap-4 p-3 border border-gray-200 rounded-lg hover:bg-gray-50">
                      {/* Main content area */}
                      <div className="flex items-center space-x-3 flex-grow min-w-[200px] w-full sm:w-auto"> {/* Ensure it takes space and wraps */}
                        ...
                        <div className="flex-1 min-w-0"> {/* Content Input/Editor */}
                           {/* --- EDITOR INTEGRATION --- */}
                           {lessonType === 'quiz' ? (
                               ...
                               <RichTextEditor
                                   ...
                                   onChange={(newContent: string) => updateLesson(module.id, lesson.id, { content: newContent })} // Only update content here
                               />
                           )}
                      ...
                      <div className="flex items-center space-x-2 flex-shrink-0 w-full sm:w-auto justify-end sm:justify-start"> {/* Justify end on mobile */}
                        <select
                          value={lessonType}
                          onChange={(e) => updateLesson(module.id, lesson.id, { type: e.target.value as any, content: '', title: `New ${e.target.value.charAt(0).toUpperCase() + e.target.value.slice(1)}`, quizId: undefined })}
                        ...
                        <div className="flex items-center space-x-1 sm:space-x-2 flex-shrink-0">
                             {lessonType === 'quiz' && (
                                 <button onClick={(e) => { e.stopPropagation(); handleOpenQuizModal(lesson); }} className={`p-1 sm:p-1.5 rounded-lg text-xs sm:text-sm font-body font-medium transition-colors flex items-center space-x-1 ${lesson.quizId ? 'bg-green-100 text-green-600 hover:bg-green-200' : 'bg-secondary-100 text-secondary-600 hover:bg-secondary-200'}`} title={lesson.quizId ? 'Edit Quiz Questions' : 'Create Quiz Questions'}>
                                     ...
                                     <span className="hidden sm:inline">{lesson.quizId ? 'Edit' : 'Add'} Quiz</span> {/* Shorter text */}
                                 </button>
                             )}
                             <button className="text-gray-400 hover:text-gray-600 p-1 cursor-move hidden sm:block"> {/* Hide move on mobile */}
        ...
        <div>
          ...
          <div className="space-y-3">
            {courseData.requirements.map((req) => (
              <div key={req.id} className="flex items-center space-x-2 sm:space-x-3"> {/* Less space on mobile */}
                <input
        ...
        <div>
          ...
          <div className="space-y-3">
            {courseData.learningOutcomes.map((outcome) => (
              <div key={outcome.id} className="flex items-center space-x-2 sm:space-x-3"> {/* Less space on mobile */}
                <input
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 py-4">
              ...
              <div className="flex items-center space-x-2 w-full md:w-auto justify-start flex-wrap gap-2 md:flex-nowrap md:gap-0 md:space-x-4 flex-shrink-0">
                ...
                >
                  ...
                  <span className="hidden sm:inline">{isLoading ? '' : (currentCourseId ? " Course" : " Course")}</span> {/* Add back "Course" on sm+ */}
                </button>
              </div>
            </div>
          </div>
