    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const [
          ...
          axios.get<{ success: boolean; data: Enrollment[] }>(
            `${API_BASE_URL}/enrollments`,
            { headers }
          ),
          axios.get<{ success: boolean; data: Certificate[] }>(
            `${API_BASE_URL}/certificates`,
            { headers }
          ),
          axios.get<{ success: boolean; data: Activity[] }>(
            `${API_BASE_URL}/activities?limit=5`,
            { headers }
          ),
          axios.get<{ success: boolean; data: Achievement[] }>(
        ...
        if (
          !enrollmentsRes.data.success ||
          !certificatesRes.data.success ||
          !activitiesRes.data.success ||
          !achievementsRes.data.success
        ) {
        ...
        setAchievements(
          allAchievements.map((ach) => ({ ...ach, earned: true }))
        ); // <-- Set achievements state
        ...
        if (axios.isAxiosError(err)) {
          setError(`Network Error: ${err.message}. Please try refreshing.`);
            ...
            <Link
              to={
                recentCourses.length > 0
                  ? `/learn/course/${recentCourses[0].course._id}`
                  : "/courses"
              }
              className="bg-secondary-500 hover:bg-secondary-600 text-white px-6 py-3 rounded-lg font-body font-semibold transition-colors"
            >
              {recentCourses.length > 0
                ? "Continue Learning"
                : "Explore Courses"}
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-xl shadow-sm border p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <Link
                  to="/my-courses"
                  className="text-primary-500 hover:text-primary-600 font-body font-medium"
                >
                  {" "}
                  View All{" "}
                </Link>
              ...
              <div className="space-y-4">
                {recentCourses.length > 0 ? (
                    ...
                    >
                      <img
                        src={
                          ...
                          `https://placehold.co/160x90/006747/FFF?text=${encodeURIComponent(
                      ...
                      <div className="flex-1">
                        <h4 className="font-body font-semibold text-gray-900 mb-1 line-clamp-1">
                          {" "}
                          {enrollment.course.title}{" "}
                        </h4>
                      ...
                      <Link
                        to={`/learn/course/${enrollment.course._id}`}
                        className="bg-primary-500 hover:bg-primary-600 text-white p-2 rounded-lg transition-colors"
                        aria-label={`Continue learning ${enrollment.course.title}`}
                      >
                  ...
                  <div className="text-center py-8">
                    ...
                    <Link
                      to="/courses"
                      className="mt-4 inline-block bg-secondary-500 text-white px-6 py-2 rounded-lg font-body font-semibold hover:bg-secondary-600"
                    >
                      {" "}
                      Explore Courses{" "}
                    </Link>
            ...
            <div className="bg-white rounded-xl shadow-sm border p-6">
              ...
              <div className="space-y-4">
                {activities.length > 0 ? (
                  ...
                  <p className="text-sm text-gray-500 font-body text-center py-4">
              ...
              <div className="space-y-4">
                {deadlines.length > 0 ? (
                  deadlines.map((deadline, index) => (
                    <Link
                      to={`/learn/course/${deadline.courseId}`}
                      key={index}
              ...
              <div className="space-y-4">
                {achievements.length > 0 ? (
                  achievements.map((achievement) => (
                    <div
                      key={achievement._id}
                      className="flex items-center space-x-3 p-3 rounded-lg bg-secondary-50 border border-secondary-200"
                    >
              ...
              <div className="text-center">
                ...
                <p className="text-accent-100 font-body">
                  {user?.loginStreak && user.loginStreak > 1
                    ...
                    : "Log in tomorrow to build your streak!"}
                </p>
              </div>
            </div>
          </div>
